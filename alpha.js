...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = alpha.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© King Of Bear Botz Whatsapp_*", "thumbnail": ftex, "surface": 'CATALOG' }}, {quoted:ftex})
             alpha.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = alpha.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `King Of Bear Botz Whatsapp`, "footerText": "*_© King Of Bear Botz Whatsapp_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© King Of Bear Botz Whatsapp_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
				...
				for (let ment of mek.message.extendedTextMessage.contextInfo.mentionedJid) {
					if (ment === alpha.user.jid){
                        ...
                        addafk(mek.key.remoteJid)
						heheh = ms(Date.now() - waktuafk)
			       sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© King Of Bear Whatsapp_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
					}
				}
						}
					}
        ...
        const sendWebp = async(from, url) => {
	        ...
	        const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + `ORG:Developer King Of Bear Whatsapp\n` + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
		...
		const reply = (teks) => {
			alpha.sendMessage(from, teks, text, { thumbnail: ofrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} WIB - ${week}`,body:"DON'T CLICK HERE",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://chat.whatsapp.com/HAZ6yFgCafUAeDbNH33IrL`}}})
		}
		const simir = (teks) => {
...
sendButMessage(from, `${JSON.stringify(me, null, 2)}`, "*_©King Of Bear*", [{buttonId: 'igm', buttonText: {displayText: 'Instagram'}, type: 1},{buttonId: 'ytm', buttonText: {displayText: 'YouTube'}, type: 1}], {quoted:ftex, contextInfo: { forwardingScore: 508, isForwarded: true}})
}
if(isButton == 'igm'){
...
quotesnya = await fetchJson(`https://api.lolhuman.xyz/api/random/quotes?apikey=88f7e906f709e555755ed4ce`)
...
//LUPA
 ...
 listMsg = {
 buttonText: 'LIST GABUT',
 footerText: 'King Of Bear Botz✓',
 description: `Hai kak☺️ Silahkan pilih menu gabut disini,Gunakan Dengan Bijak Ya kakk`,
 sections: [
                     {
                      "title": `${jmn} - ${week} - ${calender}`,
 rows: [
...
case 'gachacecan':
           ...
           sticWait(from)
           bo = args.join(' ')
           cecanz = await getBuffer(`https://api.xteam.xyz/randomimage/cewe?APIKEY=KingOfBear`)
...
case 'gachacogan':
           ...
           sticWait(from)
           bo = args.join(' ')
           coganz = await getBuffer(`https://api.xteam.xyz/randomimage/cowo?APIKEY=KingOfBear`)
...
if (args.length < 1) return random("Udin")
			saipul = args[0]
		    nyz6 = await fetchJson(`https://api.xteam.xyz/primbon/artinama?q=${body.slice(10)}&APIKEY=KingOfBear`)
		    nyz7 = `[ *ARTI NAMA* ]\n`,
...
//LUPA
  ...
  if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
					...
					teksnya = args.join(" ")
					sticWait(from)
					anu = await fetchJson(`https://api.xteam.xyz/textpro/holographic3d?text=${body.slice(9)}&APIKEY=KingOfBear`)
...
tahta = args.join(" ")
sticWait(from)
tahta = await getBuffer(`https://api.xteam.xyz/tahta?text=${body.slice(7)}&APIKEY=KingOfBear`)
...
snow = args.join(" ")
sticWait(from)
snow = await getBuffer(`https://api.xteam.xyz/textpro/snowtext?text=${body.slice(6)}&APIKEY=KingOfBear`)
...
case 'memeindo':
           ...
           sticWait(from)
           bo = args.join(' ')
           meme = await getBuffer(`https://api.xteam.xyz/randomimage/meme?APIKEY=KingOfBear`)
...
case 'darkjoke':
           ...
           sticWait(from)
           bo = args.join(' ')
           darkjoke = await getBuffer(`https://api.xteam.xyz/randomimage/darkjoke?APIKEY=KingOfBear`)
...
ini_list.push({
"displayName": `Developer King Of Bear Whatsapp`,
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `King Of Bear Whatsapp`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
"templateMessage": {
  ...
  "hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          "displayText": `Script King Of Bear Whatsapp`,
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    alpha.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`King Of Bear Whatsapp`}, MessageType.location)
...
break
            ...
            break
				case 'attp':
              if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: mek})
					if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp King Of Bear Whatsapp`)
...
var nn = body.slice(9)
           ...
           case 'fitnahpc':
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              alpha.groupUpdateSubject(from, `HACKED BY King Of Bear Whatsapp`)
...
for (let i = 0; i < args[0]; i++) {
alpha.sendMessage(from, `King Of Bear Whatsapp`, MessageType.extendedText,{
 quoted: {
key: {
...
if (args.length < 1) return reply('Jumlahnya?')
		        try {
		        var hets = `King Of Bear Whatsapp`
...
alpha.sendMessage(from, `King Of Bear Whatsapp`, text, {
 quoted: {
  key: {
   participant: '0@s.whatsapp.net' // Fake sender Jid
  },
 ...
 quoted: {
  ...
  message: {
   orderMessage: {
    ...
    surface: 1,
    message: `${NamaOwner}`,
    orderTitle: `King Of Bear Whatsapp`, // 
    sellerJid: '0@s.whatsapp.net' // Seller
   }
  }
 }
...
const btngrass = {
    contentText: `${anu}`,
    footerText: '*_©King Of Bear Whatsapp_*',
    buttons: punten,
    headerType: 1
}
